...
summer = read.table("Stavoren_daily_2019_revised.txt", sep="\t", header=TRUE)
summer_2020 = read.table("Stavoren_daily_2020_revised.txt", sep="\t", header=TRUE)
...
for(i in c("sums","mult")){
  ...
  # coef(leaps, 1:length(res_leaps$rsq)) 
  ...
  # Add column to specify if the model is significant (0 is significant, 1 is insignificant)
  models$Pval = 0
  ...
  if (combi == "mult") {
    ...
    # Check on significance (p-value < 0.05) of the model. Print which model is insignificant.
    for(k in 1:31){
      fit = lm(summer$LE ~ summer[,which(res_leaps$which[k,2:32] == TRUE)+1])
      if(lmp(fit)>= 0.05){models$Pval[k]=1; print(lmp(fit)); print(which(res_leaps$which[k,2:32] == TRUE))}
    }
  }
  
  if (combi == "sums") {
    ## Check for overall significance of the models and print models which are insignificant
    # For models with 1 variable
    for(k in 1:5){
      fit = lm(summer$LE ~ summer[,which(res_leaps$which[k,2:6] == TRUE)+1])
      if(lmp(fit)>= 0.05){models$Pval[k]=1;print(lmp(fit));print(which(res_leaps$which[k,2:6] == TRUE))}
    }
    # Models with 2 variables
    for(k in 6:15){
      fit = lm(summer$LE ~ summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[1]] + summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[2]])
      if(lmp(fit)>= 0.05){models$Pval[k]=1;print(lmp(fit));print(which(res_leaps$which[k,2:6] == TRUE))}
    }
    # Models with 3 variables
    for(k in 16:25){
      fit = lm(summer$LE ~ summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[1]] + summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[2]] + summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[3]])
      if(lmp(fit)>= 0.05){models$Pval[k]=1;print(lmp(fit));print(which(res_leaps$which[k,2:6] == TRUE))}
    }
    # Models with 4 variables
    for(k in 26:30){
      fit = lm(summer$LE ~ summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[1]] + summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[2]] + summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[3]] + summer[,(which(res_leaps$which[k,2:6] == TRUE)+1)[4]])
      if(lmp(fit)>= 0.05){models$Pval[k]=1;print(lmp(fit));print(which(res_leaps$which[k,2:6] == TRUE))}
    }  
    # Model with 5 variables
    fit = lm(LE~Kin+u+De+VPD+Twater, data=summer)
    if(lmp(fit)>= 0.05){models$Pval[31]=1;print(lmp(fit));print(which(res_leaps$which[k,2:6] == TRUE))}
...
# The model with the highest R2 value shows in the summary (res_leaps) that adding the individual terms of the model are not all significant
fit <- lm(LE ~ poly(summer[,(which.max(quadr[1,])+1)], 2, raw = TRUE), data = summer)
res_leaps = summary(fit)

# The model with the second highest R2 value shows all terms to be significant. That's why this model is chosen.
#### Daily - simple/best model ####
fit <- lm(LE ~ u + Twater, data = summer)
res_leaps = summary(fit)
coeff=coefficients(fit)
summer$LE_sim = coeff[2]*summer$u + coeff[3]*summer$Twater + coeff[1]
corr = lm(LE~LE_sim, data=summer)
round(summary(corr)$r.squared, 2)

# Calculate simulated LE in 2020 using the regression model developed with data from 2019
coeff=coefficients(fit)
summer_2020$LE_sim = coeff[2]*summer_2020$u + coeff[3]*summer_2020$Twater + coeff[1]
