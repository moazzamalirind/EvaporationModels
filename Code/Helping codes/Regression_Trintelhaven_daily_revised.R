...
summer = read.table("Trintelhaven_daily_2019_revised.txt", sep="\t", header=TRUE)
summer_2020 = read.table("Trintelhaven_daily_2020_revised.txt", sep="\t", header=TRUE)
...
# Calibration done based on the summer with the most available data points which is 2020.
# Validation is done based on the summer of 2019.
...
for(i in c("sums","mult")){
  ...
  if (combi == "sums") {
    ...
    # Model with 5 variables gives NA value, so make Pval = 1
    models$Pval[31]=1
  }  
...
fit <- lm(LE ~ u+De, data=summer)
res_leaps = summary(fit)
coeff=coefficients(fit)
summer$LE_sim = coeff[2]*summer$u + coeff[3]*summer$De + coeff[1]
corr = lm(LE~LE_sim, data=summer)
...
coeff=coefficients(fit)
summer_2020$LE_sim = coeff[2]*summer_2020$u + coeff[3]*summer_2020$De + coeff[1]
