...
summer = read.table("Trintelhaven_hourly_2019_routine_revised.txt", sep="\t", header=TRUE)
summer_2020 = read.table("Trintelhaven_hourly_2020_routine_revised.txt", sep="\t", header=TRUE)
...
quadr[1,which.max(quadr[1,])]                  # This exists per definition of max two variables
...
#### Hourly - best model ####
models_sum[1,which(models_sum[1,1:5]==1)] # Show which variables are included
# Enter the names of these variables below
fit_best <- lm(LE ~ u + De + VPD, data = summer)
res_leaps_best = summary(fit_best)
coeff=coefficients(fit_best)

# R2 - calibration (LE~LE_sim)
summer$LE_sim = coeff[2]*summer$u + coeff[3]*summer$De + coeff[4]*summer$VPD + coeff[1]
corr = lm(LE~LE_sim, data=summer)
round(summary(corr)$r.squared, 2)

# Calculate simulated LE in 2020 using the regression model developed with data from 2019
summer_2020$LE_sim_best = coeff[2]*summer_2020$u + coeff[3]*summer_2020$De + coeff[4]*summer_2020$VPD + coeff[1]
corr = lm(LE~LE_sim_best, data=summer_2020)
round(summary(corr)$r.squared, 2)
